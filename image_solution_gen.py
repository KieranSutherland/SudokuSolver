...
def generate_final_solution_img(solution_grid_img, original_frame, corners, destination):
    solution_warped = unwarp(solution_grid_img, original_frame, corners, destination)
    merged = merge_pics(original_frame, solution_warped)
    return merged

def unwarp(solution_grid_img, original_frame, corners, destination):
    M = cv2.getPerspectiveTransform(destination, corners)
    width, height, _ = original_frame.shape
    warp = cv2.warpPerspective(solution_grid_img, M, (height, width))
    return warp
