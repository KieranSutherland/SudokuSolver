...
def get_grid_img(capture):
    ...
    thresh = cv2.adaptiveThreshold(blur,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV,23,4)
    ...
    for c in contours:
        ...
        transformed = perspective_transform(thresh, approx)
        resized = resize_img(transformed)
        return resized, frame
    return None, frame

def get_individual_number_imgs(grid_img, original):
...
def resolve_numbers(grid_number_imgs):
    digits_model = tf.keras.models.load_model("models/digits_model")
    cv2.imshow('prediction image', grid_number_imgs[0][0])
    print(digits_model.predict(grid_number_imgs[0][0]).shape)
    return ""
...
def main():
    ...
    try:
        while True:
            grid_img, original = get_grid_img(capture)
            if grid_img is not None:
                cv2.imshow('grid', grid_img)
            grid_number_imgs = get_individual_number_imgs(grid_img, original)
            resolved_number_grid = resolve_numbers(grid_number_imgs)
            key = cv2.waitKey(1)
